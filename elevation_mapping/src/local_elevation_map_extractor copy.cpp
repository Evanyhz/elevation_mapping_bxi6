...
public:
    LocalElevationMapExtractor() : Node("local_elevation_map_extractor")
    {
        ...
        this->declare_parameter("robot_frame", "aft_mapped_upright");
        this->declare_parameter("map_frame", "map");
        this->declare_parameter("local_map_size_x", 1.6);
        this->declare_parameter("local_map_size_y", 1.0);
        this->declare_parameter("local_map_resolution", 0.0);  
        this->declare_parameter("update_rate", 50.0);
        this->declare_parameter("tf_timeout", 0.1);
        this->declare_parameter("use_rotated_robot_frame", true);  
        this->declare_parameter("invert_height", false);  
        this->declare_parameter("enable_resampling", false);  
        this->declare_parameter("resampling_method", "mean"); 
        this->declare_parameter("enable_nan_filling", false);  
        ...
        try {
            ...
            if (use_rotated_robot_frame_) {   // 启用坐标跟随
                ...
                if (!extended_map.getLayers().empty()) {
                    ...
                    try {
                        double sampling_factor = 0.25;  // 采样密度因子，影响局部高程图的精度
                        ...
                        RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                                   "Published rotated local elevation map at (%.2f, %.2f) with yaw %.2f in %s frame%s (extend_factor=%.1f) - Z-invariant heights", 
                ...
                RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                           "Published local elevation map at (%.2f, %.2f) in %s frame (translation only) - Z-invariant heights", 
